...
class DatabaseMockService {
  ...
  DatabaseMockService({this.uid, this.groupId});

  //collection references
  final firestore = FakeFirebaseFirestore();

  Future<void> changeUid(String uidChange) async {
    uid = uidChange;
  }
  ...
  Future<int> getUserGroupSize() async {
    var userGroupID = await firestore
        .collection("users")
        .doc(uid)
        .get()
        .then((value) => value["groupId"]);
    if (userGroupID == "") {
      return 1;
    } else {
      var userIDS = await firestore
          .collection("groups")
          .doc(userGroupID)
          .get()
  ...
  Future<void> disbandGroup(String groupUID) async {
    try {
      final QuerySnapshot searchedJoinCode = await firestore
          .collection("users")
      ...
      for (var doc in documents) {
        ...
        await firestore.collection("users").doc(doc.id).update({'groupId': ''});
      }
  ...
  Future<void> addUserToGroup(String joinCode) async {
    try {
      ...
      final DocumentSnapshot getuserdoc =
          await firestore.collection("groups").doc(groupDocID).get();

      await firestore.collection("groups").doc(groupDocID).update({
  ...
  Future<void> createGroup(String groupName) async {
    ...
    members.add(uid);
    try {
      DocumentReference _docref = await firestore.collection("groups").add({
        'leaderUID': uid,
        'listOfUsers': members,
        'routeID': '',
        'groupName': groupName,
      ...
      await firestore.collection("groups").doc(_docref.id).update({
  ...
  Future<void> deleteUser() async {
    String docref = firestore.collection("users").doc(uid).id;
    DocumentReference _docref = firestore.collection("users").doc(uid);
